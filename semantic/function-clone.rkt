...
(define (cfg-replace-function cfg fn)
  (struct-copy control-flow-graph cfg
               [functions
                (ordered-map-set (control-flow-graph-functions cfg)
                                 (asm-function-id fn)
                                 fn)]
               [fn-names
                (ordered-map-set (control-flow-graph-fn-names cfg)
...
(define (cfg-rewrite-call-targets cfg old-target new-target
  ...
  (for/fold ([cfg* cfg])
            ([kv (in-ordered-map (control-flow-graph-functions cfg))])
    (define fn (cdr kv))
...
(define (all-existing-function-names cfg)
  (for/set ([kv (in-ordered-map (control-flow-graph-functions cfg))])
