...
")
    ...
    (check-true (bitset-member? caller-def 0)))
...
")
    ...
    (check-true (bitset-member? outer-def 0))))
...
")
    ...
    (check-true (bitset-member? root-def 0))
    (check-true (bitset-member? root-def 1)))
...
"))
    ...
    (check-true (bitset-member? def1 0))
    (check-false (bitset-member? def1 5))
    (check-true (bitset-member? def2 5))
    (check-false (bitset-member? def2 0))))
...
"))
    ...
    (check-false (bitset-member? def1 30))
    ;; leaf: x30 不是 preserved，在 scratch 中
    (check-true (bitset-member? def2 30))
