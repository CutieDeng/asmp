...
static int roundtrip(const char *name,
  ...
  uint64_t out_cap = asmp_deflate_neon_extend_raw_bound((uint64_t)len);
...
static int status_checks(void) {
  ...
  uint64_t bound = asmp_deflate_neon_extend_raw_bound(src_len);
  uint64_t scratch_len = asmp_deflate_neon_extend_raw_scratch_size();
  ...
  int status = asmp_deflate_raw_fixed_neon_extend(NULL, bound, &out_len, src,
                                                  src_len, scratch, scratch_len);
  failed |= status != ASMP_DEFLATE_BAD_ARGUMENT;

  out_len = 99;
  status = asmp_deflate_raw_fixed_neon_extend(dst, 1, &out_len, src, src_len,
                                              scratch, scratch_len);
  failed |= status != ASMP_DEFLATE_DST_TOO_SMALL || out_len != 0;

  out_len = 99;
  status = asmp_deflate_raw_fixed_neon_extend(dst, bound, &out_len, src, src_len,
                                              scratch, 16);
  failed |= status != ASMP_DEFLATE_SCRATCH_TOO_SMALL || out_len != 0;

  out_len = 99;
  status = asmp_deflate_raw_fixed_neon_extend(dst, bound, &out_len, NULL, 0,
                                              scratch, scratch_len);
