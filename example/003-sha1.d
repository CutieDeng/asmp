;; ============================================================
;; 003-sha1.d - ARM NEON SHA1（寄存器分配验证版）
;; ============================================================
;;
;; SHA1 API:
...
;;
;; 运行: racket cli/as.rkt example/003-sha1.d
...
(: label entry)
  ;; 使用虚拟寄存器保存循环状态，验证寄存器分配器在非叶子函数中的正确性
  (: save! all)
  (mov x29 sp)
...
(: label loop_pair)
  ...
  (sub x.blocks x.blocks 2)
  (cbnz x.blocks loop_pair)
...
;;
;; 说明: ARM SHA1 指令
;;   sha1c/sha1p/sha1m 更新 ABCD
;;   新 E 路径使用 sha1h（基于旧 A）并通过虚拟寄存器参与分配
;;
;; 虚拟寄存器:
...
;;   v.wk       = W + K
;;   v.k0-v.k3  = K 常量
;;   s.e_tmp    = E 传递给 sha1c/p/m，也用于 sha1h 临时值
...
(: label entry)
  ...
  (fmov s.e_tmp w.a_tmp)
  (sha1h s.e_tmp s.e_tmp)
  (fmov w.e s.e_tmp)
