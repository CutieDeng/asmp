...
(define (reg-key-compare a b)
  (define class-cmp (symbol-compare (car a) (car b)))
  (match class-cmp
...
(struct save-load-context
  (slot-table         ; ordered-map[reg-key → slot-info] - 全局槽分配
...
(define (analyze-save-load fn alloc-result [config default-save-load-config]
                            #:abi [abi arm64-abi])
  ;; assignment : ordered-map[reg-id → color]
  (define assignment (alloc-result-assignment alloc-result))
  ;; coalesced : ordered-map[reg-id → reg-id]
  ...
  (define all-reg-keys
    (for/list ([kv (in-ordered-map all-regs-map)])
      (car kv)))
...
(define (allocate-slots reg-keys config)
  ...
  (define-values (table-after-gpr offset-after-gpr)
    (for/fold ([tbl (ordered-map-empty reg-key-compare)]
               [off 0])
              ([key (in-list sorted-gpr)])
      (values (ordered-map-set tbl key (slot-info off 8 'gpr))
              (+ off 8))))
  ...
  (define-values (table-after-fpr offset-after-fpr)
    (for/fold ([tbl table-after-gpr]
      ...
      (values (ordered-map-set tbl key (slot-info off 16 'fpr))
              (+ off 16))))
  ...
  (define-values (table-after-sve sve-slot-count)
    (for/fold ([tbl table-after-fpr]
      ...
      (values (ordered-map-set tbl key (slot-info idx 'vl sve-class))
              (add1 idx))))
...
;; fn           : asm-function
;; alloc-result : alloc-result (含 assignment ordered-map)
;; abi          : abi-config
;; class        : 'gpr | 'fpr | 'sve-z | 'sve-p
...
(define (collect-used-callee-saved fn alloc-result abi class)
  ...
  (if (not callee-saved)
      '()
      (let ()
        ;; 使用 bitset 收集已使用的 callee-saved 寄存器编号
        ;; 寄存器编号为小非负整数 (0-31)，适合 bitset 表示

        ;; 初始集合: GPR 始终包含 x29 (FP), x30 (LR)
        (define initial-regs
          (if (eq? class 'gpr)
              (bitset-add (bitset-add bitset-empty 29) 30)
              bitset-empty))
        ...
        ;; 扫描直接使用的物理寄存器
        ;; regs-final : bitset — 所有已使用的 callee-saved 寄存器编号
        (define regs-final
        ...
        ;; bitset 自然有序，直接转为升序列表
        (for/list ([i (in-bitset regs-final)])
          i))))
    ...
    (for/fold ([blocks (asm-function-blocks fn)])
      ...
      (define new-block
        (expand-block block bb-val slot-table total-size sve-slots
                      first-save load-locs all-expansion resolved-regs config))
      (ordered-map-set blocks bb-val new-block)))
...
(define (generate-store-pairs result keys slot-table kind use-paired?)
  ...
  (define sorted-keys
    (sort keys <
          #:key (lambda (k)
                  (slot-info-offset (ordered-map-ref slot-table k #f)))))
  ...
  (let loop ([ks sorted-keys] [res result])
    (match ks
      ...
      [(list k1 k2 rest ...)
       #:when use-paired?
       (define slot1 (ordered-map-ref slot-table k1 #f))
       (define slot2 (ordered-map-ref slot-table k2 #f))
      ...
      [(cons k rest)
       (define slot (ordered-map-ref slot-table k #f))
  ...
  (define can-use-post-index?
    (and is-last?
         ...
         (let ()
           ...
           (match sorted
             [(list k1 k2 _ ...)
              (define off1 (slot-info-offset (ordered-map-ref slot-table k1 #f)))
              (define off2 (slot-info-offset (ordered-map-ref slot-table k2 #f)))
              (and (= off1 0) (= off2 8))]
             [_ #f]))))  ; 连续且从 0 开始
    ...
    (if (not (and use-post-index? (>= (length sorted-keys) 2)))
        ...
        (match sorted-keys
          [(list k1 k2 rest ...)
           (define off1 (slot-info-offset (ordered-map-ref slot-table k1 #f)))
           (define off2 (slot-info-offset (ordered-map-ref slot-table k2 #f)))
           (define slot-size (slot-info-size (ordered-map-ref slot-table k1 #f)))
