...
(define (parse-abi-props-with-inherit name props raw-hash parsed-hash)
  ...
  (when parent-abi
    ...
    (define gpr-missing
      (for/list ([r (in-bitset (bitset-subtract (abi-effect-gpr-scratch parent-eff)
                                                (abi-effect-gpr-scratch child-eff)))])
        r))
    (define fpr-missing
      (for/list ([r (in-bitset (bitset-subtract (abi-effect-fpr-scratch parent-eff)
                                                (abi-effect-fpr-scratch child-eff)))])
        r))
    (define pred-missing
      (for/list ([r (in-bitset (bitset-subtract (abi-effect-pred-scratch parent-eff)
...
(define (parse-class-config-new class-name num-regs props parent-cfg)
  (define banned bitset-empty)
  (define preserved bitset-empty)
  (define arg-regs '())
  (define return-regs '())
  ...
  (for ([p (in-list props)])
    (match p
      [(list 'banned val)
       (set! banned (integer->bitset val))]
      [(list 'preserved val)
       (set! preserved (integer->bitset val))]
      [(list 'args regs ...)
       (set! arg-regs regs)]
      [(list 'return regs ...)
